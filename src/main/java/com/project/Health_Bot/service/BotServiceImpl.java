...
public class BotServiceImpl implements BotService {
	...
	/**
	 * Se è true significa che l'utente ha richiesto le info nutrizionali di un
	 * alimento
	 */
	...
	public List<SendMessage> gestisciUpdate(Update update) throws InvalidUpdateException {
		...
		if (update.message() != null && !update.message().text().isEmpty()) { // Messaggio ricevuto valido
			...
			if (utenteNonRegDao.isRegistering(userId)) {
				// Utente in fase di registrazione, è presente nell'hash_map degli utenti che si
				// stanno registrando
				return gestisciReg(update.message().text(), userId, chatId);
			} else if (utenteRegDao.isRegistered(userId)) {
				// Utente già registrato nel sistema, è presente nell'hash_map degli utenti che
	...
	public List<SendMessage> gestisciReg(String mess, String userId, long chatId) {
		...
		// Individuo quale campo deve essere ancora compilato e restituisco la vista
		// corrispondente
	...
	public List<SendMessage> gestisciMenu(String mess, String userId, String username, long chatId) {
		...
		// Verificare che il tipo di utente non sia lo stesso del pulsante premuto (in
		// tal caso non fare nulla)
		case "Pesante 🏋🏻":
			if (tipo != "pes") {
